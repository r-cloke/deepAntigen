...
def create_placeholders(n_x, n_y):
    X = tf.placeholder(tf.float32,name="X",shape=(n_x,195))
...
def initialize_parameters():   
    ...
    W1 = tf.get_variable("W1", [50,195], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    b1 = tf.get_variable("b1", [50,1], initializer = tf.zeros_initializer())
    W2 = tf.get_variable("W2", [1,50], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
...
def compute_cost(Z2, Y):
    ...
    cost = tf.losses.mean_squared_error(labels=logits,predictions=Y)
 
    return cost

def model(X_train, Y_train, X_test,learning_rate,num_epochs, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        test_X = tf.placeholder(tf.float32,name="test_X",shape=(X_test.shape[0],195))
...
def getXData():
    ...
    lines = open(path + '\\seq_binding_phys_IEDB.txt').readlines()[1:]
    random.shuffle(lines)

    for i in range(len(lines)):
        physico = lines[i].split('\t')[2:17]
        physico = [s.replace('\n', '') for s in physico]
        physLst.append(physico)
        
        seq = lines[i].split('\t')[0].strip('\n')
        binaryPeptide = binaryEncode(seq)
        pepLst.append(binaryPeptide)
            
        response = lines[i].split('\t')[1].strip('\n')
    ...
    physArr = np.array(physLst)

    X = np.concatenate((X,physArr),axis=1)
...
X_train = Xdataset[0:4832,0:195]
X_test = Xdataset[4832:6039,0:195]
...
parameters = model(X_train, Y_train, X_test,learning_rate = 0.001,num_epochs = 50, print_cost = True)